...
public sealed class InteractionMapBakerV2 : MonoBehaviour
{
    ...
    [SerializeField] private int _dirtyFrames = 3;
    [SerializeField] private bool _autoDirtyOnMove = true;
    [SerializeField] private int _autoDirtyFramesOnMove = 3;
        ...
        Vector3 arcForward)
    {
        ...
        if (_stampQueue.Count >= 256)
        {
            _stampQueue.Dequeue(); // drop oldest
        }
    ...
    private void Tick()
    {
        ...
        // Force processing if stamps are queued regardless of tick policy
        if (hasPendingStamps && _eventDrivenTick)
            MarkDirty(_dirtyFrames);
